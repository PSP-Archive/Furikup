...
#define getSocketErrorCode sceNetInetGetErrno

char buffer[20];
char *getSocketError(void)
{
	sprintf(buffer, "%08X", getSocketErrorCode());
...
static ortp_socket_t create_and_bind(const char *addr, int port, int *sock_family){
	...
	*sock_family=AF_INET;
	...
	if (sock>=0)
	{
		...
		/* increase RTP buffer on windows and on the PSP */
		int bufsize = 32768;
    ...
    switch (session->rtp.sockfamily) {
        case AF_INET: {
			...
			retval= setsockopt(session->rtp.socket, IPPROTO_IP, IP_MULTICAST_TTL,
						   &session->multicast_ttl, sizeof(session->multicast_ttl));
    ...
    switch (session->rtp.sockfamily) {
        case AF_INET: {
			...
			retval= setsockopt(session->rtp.socket, IPPROTO_IP, IP_MULTICAST_LOOP,
						    &session->multicast_loopback, sizeof(session->multicast_loopback));
...
int rtp_session_set_dscp(RtpSession *session, int dscp){
	...
	switch (session->rtp.sockfamily) {
		case AF_INET:
		retval = setsockopt(session->rtp.socket, IPPROTO_IP, IP_TOS, &tos, sizeof(tos));
		break;
#ifdef ORTP_INET6
	case AF_INET6:
